...
class Tf_train_ctc(object):
    ...
    def set_model_name(self):
        ...
        self.selected_model = []
        for i in range(most_ep,0,-10):
            MODEL_NAME = "model.ckpt-"+str(i)
            self.selected_model.append(MODEL_NAME)                   
    ...
    def run_model(self):
        ...
        with self.graph.as_default(), tf.device('/cpu:0'):
            ...
            for MODEL_NAME in self.selected_model:
                MODEL_PATH = os.path.join(self.SESSION_DIR, MODEL_NAME)
                logger.info(section.format('Decoding test data ' + MODEL_NAME))
